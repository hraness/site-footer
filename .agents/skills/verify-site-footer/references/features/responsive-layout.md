...
- Shared organization attribution (“Built by Hraness” plus its subtitle) in every context: exact copy, one `lang="en"` block with no link, never `display: none`; out of flow with no footprint below 47.5rem; at wide widths it is a row-height box centered with the socials, its title joins the named layout once its track reaches 7rem, the subtitle follows at 40rem, no revealed line overflows, and the wide no-signup and account rows reveal both lines at 1280.
