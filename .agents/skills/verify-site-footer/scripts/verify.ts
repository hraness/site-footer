...
export function createLayoutContract(
  ...
  if (names.has("support")) {
    add({ box: "support", id: `${viewport}.support.minimum`, kind: "minimum-size", minimumHeight: 28, minimumWidth: 28 });
...
// The verifier states the expected organization lockup itself rather than
// importing it, so a source change to the brand copy is caught here.
const BRAND_NAME = "by Hraness";

// Browser-side statements shared by every context. They throw when the home
// link loses its exact lockup or accessible name, or when the removed
// standalone attribution block reappears.
const BRAND_STATEMENTS = `
  ...
  if (
    ...
    || brandLink.getAttribute("aria-label") !== "Hraness home"
    || brandLink.getAttribute("lang") !== "en" || brandLink.getAttribute("dir") !== "ltr"
    || brandLink.textContent !== ${JSON.stringify(BRAND_NAME)}
    || brandName.textContent !== ${JSON.stringify(BRAND_NAME)}
    || document.querySelectorAll(".hraness-site-footer__brand").length !== 1
    || document.querySelector('[data-slot="hraness-attribution"], .hraness-site-footer__attribution') !== null
    || /Ben Guo|Built by Ben|Built by Hraness/u.test(document.querySelector("#hraness-site-footer").textContent)
  ) {
    throw new Error("Footer brand lost its exact organization lockup.");
  }
`;
...
const LAYOUT_SAMPLE_EXPRESSION = `(() => {
  ...
  if (!(support instanceof HTMLAnchorElement) || support.href !== 'https://account.hraness.com/support?product=soundfish&source=web#support'
    || support.textContent !== '' || !(supportIcon instanceof SVGElement) || supportIcon.getAttribute("aria-hidden") !== "true"
    || support.target || !support.title.includes('Support ongoing development')) {
    throw new Error("Support link lost its product handoff, question icon, or optional value proposition.");
  }
  ...
  if (socialLinks.length !== 4 || brand.textContent !== "by Hraness") {
    throw new Error("Footer must show the Ra icon with the by-Hraness lockup and exactly four social links.");
  }
  ${BRAND_STATEMENTS}
