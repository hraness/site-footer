...
- Footer strings render on every portfolio site. Change a string only together with its test and a release note, and remember that sites receive it only when they upgrade the package.
- The cookie note describes what the host site does. It mentions sign-in only when the site passes `signIn`; do not add a claim that some consuming site cannot back.
- The English signup dialog names the product from `mailingList.name`. The `hraness` audience description is owner-set wording; propose changes to the owner instead of editing it.
